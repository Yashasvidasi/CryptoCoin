...
server.on("listening", () => {
  ...
  const MY_ADDRESS = `ws://${address.address}:${address.port}`;
  console.log(`Server is listening on ${MY_ADDRESS}`);
});
      ...
      case "TYPE_REPLACE_CHAIN":
        ...
        if (newBlock.prevHash !== JeChain.getLastBlock().prevHash) {
          ...
          if (
            ...
            console.log("New Block added:>>>>", JeChain);
          }
        } else if (
          !checked.includes(
            JSON.stringify([
...
async function connect(address) {
  if (
    ...
    socket.on("open", () => {
      ...
      opened.forEach((node) => {
        node.socket.send(
          JSON.stringify(produceMessage("TYPE_REQUEST_CHAIN", [address]))
        );
      });
...
setTimeout(() => {
  ...
  console.log("addming transaction");
  JeChain.addTransaction(transaction);
  console.log("transaction addeded");
}, 6000);
